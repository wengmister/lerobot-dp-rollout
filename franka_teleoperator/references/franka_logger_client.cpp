...
struct RobotStep
{
    ...
    // Actions (position commands sent to trajectory generator)
    std::array<double, 7> position_commands;  // target joint positions
...
private:
    ...
    void logRobotStep(const franka::RobotState& robot_state, const std::array<double, 7>& position_commands)
    {
        ...
        if (is_first_sample || elapsed_since_last_log >= LOG_INTERVAL_SEC)
        {
            ...
            // Actions: position commands
            for (int i = 0; i < 7; i++) {
                step.position_commands[i] = position_commands[i];
            }
    ...
    void saveJointLog()
    {
        ...
        for (const auto& step : robot_log_buffer_) {
            ...
            // Position commands (actions)
            for (int i = 0; i < 7; i++) {
                csv_file << "," << std::setprecision(6) << step.position_commands[i];
            }
...
private:
    void runVRControl(franka::Robot &robot)
    {
                                       ...
                                       franka::Duration period) -> franka::JointVelocities
        {
            ...
            // Extract target positions for logging (the high-level commands)
            std::array<double, 7> target_joint_positions;
            for (int i = 0; i < 7; i++) {
                target_joint_positions[i] = ruckig_input_.target_position[i];
            }
            
            // Log robot state and position commands
            logRobotStep(robot_state, target_joint_positions);
