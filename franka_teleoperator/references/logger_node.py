#!/usr/bin/env python3
# logger_node.py - Send robot pose commands with keyboard-controlled recording
...
class LoggerNode(Node):
    def __init__(self):
        ...
        # Recording state (controlled by keyboard)
        ...
        self.get_logger().info(f"VR to Robot Converter with Logging started")
        if self.use_tcp:
        ...
        self.get_logger().info("Press ENTER to toggle recording, Ctrl+C to exit")
    ...
    def check_keyboard_input(self):
        ...
        if sys.stdin.isatty() and select.select([sys.stdin], [], [], 0)[0]:
            ...
            if char == '\n':  # Enter key
                ...
                status = "\033[92mSTARTED\033[0m" if self.is_recording else "\033[93mSTOPPED\033[0m"
                self.get_logger().info(f"Recording {status}")
                return True
        return False

    def control_loop(self):
        # \"\"\"Main control loop - converts VR pose to robot commands with recording flag\"\"\"
        ...
        try:
            ...
            # Send robot command with recording flag
            record_flag = 1 if self.is_recording else 0
            self.send_robot_command(self.target_position, self.target_orientation, record_flag)
            ...
            if current_time - self.last_log_time >= self.log_interval:
                ...
                # Color codes: green for recording, yellow for idle
                record_status = "\033[92mRECORDING\033[0m" if self.is_recording else "\033[93mIDLE\033[0m"
                if self.vr_messages_received > 0:
                    self.get_logger().info(
                        f'VR: {vr_frequency:.1f} Hz | Status: {record_status} | '
                        f'Pos: [{self.target_position[0]:.3f}, {self.target_position[1]:.3f}, {self.target_position[2]:.3f}]'
                    )
                else:
                    # Color codes: green for recording, yellow for idle
                    record_status = "\033[92mRECORDING\033[0m" if self.is_recording else "\033[93mIDLE\033[0m"
                    self.get_logger().info(f'No VR data received | Status: {record_status} | Waiting for VR input...')
    ...
    def send_robot_command(self, position, orientation, record_flag):
        # \"\"\"Send pose command with recording flag to robot via UDP\"\"\"
        try:
            # Send pose + record flag: position (x,y,z), orientation (qx,qy,qz,qw), record (0/1)
