#!/usr/bin/env python3
"""
Franky Client for connecting to the Franka Position Server

This client provides the interface expected by the lerobot FrankaFER robot class.
...
class FrankaRobotClient:
    def __init__(self, server_ip="192.168.18.1", server_port=5000):
        ...
        self.base_url = f"http://{server_ip}:{server_port}"  # For compatibility
    ...
    def get_state(self):
        """Get current robot state (backward compatibility)"""
        response = self._send_command("GET_STATE")
        if response and response.startswith("STATE"):
    ...
    def set_position(self, positions):
        """Send position command to robot (backward compatibility)"""
        if len(positions) != 7:
            raise ValueError("Position command must have 7 values")
    ...
    def configure(self, dynamics_factor: float = 0.3) -> bool:
        """Configure robot parameters"""
        # For now, this is a no-op since the server doesn't support runtime config changes
        # The dynamics factor is applied when the server connects to the robot
        return True
